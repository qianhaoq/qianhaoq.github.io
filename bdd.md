...
- `features/steps/**/*.mjs`: Cucumber step definitions，验证构建后的 `dist/` 产物。
- `pnpm bdd`: 在 `pnpm build` 后执行，读取静态产物做验收。

## 编写规则

- 场景使用 `Given / When / Then`，避免描述实现细节、CSS 类名或组件内部结构。
- BDD 只断言公开行为：已发布文章可见、草稿不可见、搜索/RSS/sitemap 可发现、关键页面可访问。
- 新增用户可见能力时，先补或同步补一个失败的 BDD 场景，再实现。
- 修复 bug 时，如果 bug 影响公开行为，必须新增回归场景。
- BDD 不写入 repo 文件，不依赖外部网络，不验证 GitHub API；线上发布状态由 GitHub Actions 和 Pages API 验证。
